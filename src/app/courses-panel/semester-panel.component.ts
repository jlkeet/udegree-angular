...
@Component({
  ...
  <div class="semester-panel">
    <mat-toolbar class="title">
      {{semester.year}}
      {{semester.period===0? 'Summer School' : 'Semester ' + semester.period}}
    ...
    <div *ngIf="toggled" class="courses">
      <div class="flex dragula-container">
        <div class="semester-courses"  [dragula]="bagName" [dragulaModel]="courses"
             [attr.period]="semester.period"
             [attr.year]="semester.year">
          <course-draggable *ngFor="let course of courses; let i = index"
...
export class SemesterPanel {
  ...
  private ngOnInit() {
    ...
    //     container: (el) => el.classList.contains('dragula-container'),
  ...
  private onDropModel(args) {
    ...
    const droppedCourse = {id: Number(el.dataset.id), period: Number(el.dataset.period), year: Number(el.dataset.year)};
    ...
    if (!this.sameTime(droppedCourse)) {
      ...
      const moveHere = this.courses.filter((course: ICourse) => course.id === droppedCourse.id).length !== 0;
      if (!moveHere) {
  ...
  private sameTime(course: any) {
    return this.semester.year === Number(course.year) && this.semester.period === Number(course.period);
  }
  ...
  private deleteSemester() {
    ...
    semesters = semesters.filter((semester) =>
      semester.year !== this.semester.year || semester.period !== this.semester.period);
